...
def circularring(crimepath, c, d, crimelink, policelink):
    crimenode = None
    crimenodefeat = None
    cimenodecord = None
    policenode = None
    ...
    for feature in layer:
        ...
        for feat in yer:
            ...
            if feat.GetField("name") == crimelink:
                crimenode = feature.GetField("name")
                cimenodecord = (feature.GetGeometryRef().GetX(), feature.GetGeometryRef().GetY())
                crimenodefeat = feature
            elif feat.GetField("name") == policelink:
                policenode = feature.GetField("name")
                policenodecord = (feature.GetGeometryRef().GetX(), feature.GetGeometryRef().GetY())
                policenodefeat = feature
        dic[feature.GetField("name")] = ame
    return dic, crimenode, policenode, crimenodefeat, policenodefeat, cimenodecord, policenodecord


def deadendremoval(crimepath, c, d, crimelink, policelink):
    deadlist = []
    dic, crimenode, policenode, crimenodefeat, policenodefeat, cimenodecord, policenodecord = circularring(crimepath, c,
                                                                                                           d, crimelink,
                                                                                                           policelink)
    q, r = crime(crimepath)
    pointA = float((c + q) / 2)
    ...
    remove_dead_ends(crimepath, c, d, deadlist, crimelink, policelink)
    removing_higher_order_dead_end(crimepath, c, d, deadlist, crimelink, policelink)
    legitimateroadlist=roadidlist(crimepath, c, d,deadlist)
    nonlegitimatenodelist=nodelst(crimepath, c, d, deadlist)
    lit=routel(cimenodecord, policenodecord, deadlist, legitimateroadlist ,nonlegitimatenodelist)
    print("LIST OF REMOVED ROAD FEATURE IS",deadlist,"LIST OF LEGITIMATE ROAD FEATURE IS",legitimateroadlist)
    return deadlist, crimenode, policenode, crimenodefeat, policenodefeat, cimenodecord, policenodecord,lit


def remove_dead_ends(crimepath, c, d, deadlist, startlink, endlink):
    q, r = crime(crimepath)
    pointA = float((c + q) / 2)
    pointB = float((d + r) / 2)
    point = ogr.Geometry(ogr.wkbPoint)
...
def feautreid(crimepath, c, d, deadlist):
    featureid = []
    q, r = crime(crimepath)
    pointA = float((c + q) / 2)
    pointB = float((d + r) / 2)
...
def removing_higher_order_dead_end(crimepath, c, d, deadlist, startlink, endlink):
    x = True
    lila = {}
    change = {}
    change[0] = len(deadlist)
    counter = 0
    id = feautreid(crimepath, c, d, deadlist)
...
def roadidlist(crimepath,c,d,deadlist):
    legitimateroadlist=[]
    q, r = crime(crimepath)
    pointA = float((c + q) / 2)
    pointB = float((d + r) / 2)
    ...
    return legitimateroadlist

def nodelst(crimepath,c,d,deadlist):
...
def routel(cimenodecord, policenodecord,deadlist,legitimateroadlist,nonlegitimatenodelist):
    lit=[]
    ...
    layer.SetSpatialFilter(poly)
    for feat in layer:
        if feat.GetField("name") not in deadlist and feat.GetField("name") in legitimateroadlist:
            lit.append(feat.GetField("name"))
    print(lit, "LIST OF ROAD ID")
    return lit
...
def roadlink(pfeat, fcord, q, r, id, l, roadId,lit):
    kdic = {}
    start_end_cord = {}
    liladhar = []
    fid = {}
    ...
    for feature in layer:
        ...
        if feature.GetField("name") in l or feature.GetField("name") in roadId:
            continue
        if feature.GetField("name") not in lit:
            continue
    ...
    print("LIST OF ROUTE WITH LENGTH FROM NODE",pfeat.GetField("name"),"IS",fid)
...
def check(liladhar, q, r, route, id, l, start_end_cord,lit):
    ...
    while x:
        ...
        print("CITY CENTERNODE CLOSEST TO THE END POINT OF THE SELECTED ROAD FEATURE", f.GetField("name"), "IS",
              pfeat.GetField("name"))
...
def nearest_policia(path, crimepath):
    ...
    for feat in layer:
        ...
        d[length] = feat
        l.append(length)
    print(patht[min(l)], "THIS IS THE CLOSEST SERVICE AVAILABLE")
...
def shortest_path_forward(crimepath, c, d):
    cordlist = []
    finalroute = []
    roadId = []
    q, r = crime(crimepath)
    cimelink = (crimelink(crimepath, networkpath="C:/Users\Hp\Desktop\DATA\ROADNETWORK\ROADNETWORK.shp"))
    plink = policelink(c, d, networkpath="C:/Users\Hp\Desktop\DATA\ROADNETWORK\ROADNETWORK.shp")
    l, crimenode, policenode, crimenodefeat, policenodefeat, cimenodecord, policenodecord,lit = deadendremoval(crimepath, c,d, cimelink,plink)
    l.append(cimelink)
    l.append(plink)
    ...
    x = True
    while x:
        final_route, firstcord, s = roadlink(policenodefeat, policenodecord, float(cimenodecord[0]), float(cimenodecord[1]), cimelink, l, roadId,lit)
    ...
    print(
        "THE ROAD ID IS AS FOLLOWS@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@",
        roadId)
    return roadId


def shortest_path_reverse(crimepath, c, d):
    reversecordlist = []
    reversefinalroute = []
    q, r = crime(crimepath)
    flink = (crimelink(crimepath, networkpath="C:/Users\Hp\Desktop\DATA\ROADNETWORK\ROADNETWORK.shp"))
    plink = policelink(c, d, networkpath="C:/Users\Hp\Desktop\DATA\ROADNETWORK\ROADNETWORK.shp")
    l, crimenode, policenode, crimenodefeat, policenodefeat, cimenodecord, policenodecord,lit = deadendremoval(crimepath, c,
                                                                                                           d, flink,
                                                                                                           plink)
    reverseroadId = [flink]
    m, n = cimenodecord, crimenodefeat
    '''fcord,ffeat=firstcord_1(c,d)
    t=(c,d)
    xmin,ymax=a(flink,t)

    s = feature_in_buffer(crimenodefeat)
    if s=="NO FEATURE FOUND":
        m, n = firstcord_1(ymax[0], ymax[1])'''
    x = True
    while x:
        final_route, firstcord, s = roadlink(n, m, policenodecord[0], policenodecord[1], id, l, reverseroadId,lit)
        if s == "NOT OK FINISH LIST":
            print(s)
            break
        ...
        print("THE REVERSE PATH COMPRISES OF FOLLOWING LINK",reverseroadId)
...
def shortroute(crimepath, c, d):
    pathdics = {}

    print(crimepath, "THIS IS THE PATH WHERE CRIME FILE IS SAVED")
    forwardroute = shortest_path_forward(crimepath, c, d)
    print("THE FORWARD ROUTE IS AS FOLLOWS", forwardroute)
    reverseroute = shortest_path_reverse(crimepath, c, d)
    ...
    for layer in datasource:
        for feat in layer:
            if feat.GetField("name") ==flink:
                ...
                p=bufgeo((feat.GetGeometryRef().GetPoint(count - 1)[0], feat.GetGeometryRef().GetPoint(count - 1)[1]),0.00010)
...
def pointsetgenerator(policepoint,crimepoint):
    p=[]
    c=[]
    d=[]
    divc={}
    fe={}
    first=[]
    for k,v in policepoint.items():
        p.append(policepoint[k])
    for q,t in crimepoint.items():
        c.append(crimepoint[q])
    d.append(haversine(p[0],c[0]))
    d.append(haversine(p[0],c[1]))
    d.append(haversine(p[1],c[0]))
    d.append(haversine(p[1],c[1]))
    divc[haversine(p[0],c[0])]=(p[0],c[0])
    divc[haversine(p[0],c[1])] = (p[0], c[1])
    divc[haversine(p[1], c[0])] = (p[1], c[0])
    divc[haversine(p[1], c[1])] = (p[1], c[1])
    for k,v in policepoint.items():
        if policepoint[k]==divc[min(d)][0]:
           fe[divc[min(d)]] =first.append(k)
    for q,t in crimepoint.items():
        if crimepoint[q] == divc[min(d)][1]:
            fe[divc[min(d)]] = first.append(q)
        c.append(crimepoint[q])
    return divc,divc[min(d)],divc[max(d)]
...
def anbtobanjao(c,d,q,r):
    ...
    for k,v in policenodelist.items():
        policepoint[policenodelist[k]]=(k.GetGeometryRef().GetX(),k.GetGeometryRef().GetX())
    for l,m in crimenodelist.items():
        crimepoint[crimenodelist[l]]=(l.GetGeometryRef().GetX(),l.GetGeometryRef().GetX())
    pointsetgenerator(policepoint, crimepoint)

anbtobanjao(c=78.19786075987997, d=26.214123796012398, q=78.19314865853244, r=26.209821259547105)
