...
def banakerahege(c,d,q,r,fruit,nodepath,networkpath,path,name):
...
def main(c, d, q, r, path, name, fruit):
    networkpath="C:/Users\Hp\Desktop\DATA\BHOPALLINES\BHOPALLINES.shp"
    nodepath="C:/Users\Hp\Desktop\DATA\DELHINODES\DELHINODES.shp"
    froute,cli,pli=banakerahege(c, d, q, r, fruit,nodepath,networkpath,path,name)
    if len(froute)>1:
        cbuf1, cbuf2, pbuf1, pbuf2, pcord1, pcord2, ccord1, ccord2=boundingpre(cli, pli,nodepath,networkpath)
        cline, pline=boundingmains(c,d,q, r, froute, cbuf1, cbuf2, pbuf1, pbuf2, pcord1, pcord2, ccord1, ccord2, cli, pli,nodepath,networkpath)
        prefinaldest(cline, pline, cli, pli, froute, path, name,nodepath,networkpath)
    if len(froute)==1:
        a=wktsingle(froute, networkpath)
        route(a,path,name)
