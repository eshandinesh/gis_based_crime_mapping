...
def creating_directory():
    ...
    folderPath = "C:\padmanabh\smartgwalior\smartcity/tempData"
...
def police():
    datasource = open_File(path="C:\padmanabh\smartgwalior\smartcity/tempData\POLICELOCATION\POLICELOCATION.shp")
...
def remove_dead_ends(id):
    l=[]
    pointdatasource = open_File(path="C:\padmanabh\smartgwalior\smartcity/tempData/NODES/NODES.shp")
    pointlayer = pointdatasource.GetLayerByIndex(0)
    datasource = open_File(path="C:\padmanabh\smartgwalior\smartcity/tempData\ROADNETWORK\ROADNETWORK.shp")
    layer = datasource.GetLayerByIndex(0)
    for feature in pointlayer:
        layer.SetSpatialFilter(feature.GetGeometryRef().Buffer(0.00010))
        numFeatures = layer.GetFeatureCount()
        for fe in layer:
            if numFeatures == 1 and fe.GetField("name") != id and fe.GetField("name") not in l:
...
def roadlink(pfeat,fcord,q,r,id,l,roadId,froute):
    kdic={}
    start_end_cord={}
    liladhar=[]
    fid = {}
    ...
    for feature in layer:
        if feature.GetField("name") not in froute:
            continue
        if feature is None:
...
def wktlist(finalroute):
    li = []
    datasource = open_File("C:\padmanabh\smartgwalior\smartcity/tempData\ROADNETWORK\ROADNETWORK.shp")
    for layer in datasource:
        for feat in layer:
...
def shortest_path_forward(c,d,q,r,froute):
    cordlist=[]
    finalroute = []
    roadId = []
    path="C:\padmanabh\smartgwalior\smartcity/tempData\POLICELOCATION\POLICELOCATION.shp"

    #c,d=police()

    id=(crimelink(q,r,networkpath="C:\padmanabh\smartgwalior\smartcity/tempData\ROADNETWORK\ROADNETWORK.shp"))
    ...
    x=True
    while x:
        final_route,firstcord,s=roadlink(pfeat,firstcord,float(q),float(r),id,l,roadId,froute)
...
def shortest_path_reverse(q,r,c,d,froute):
    reversecordlist=[]
    reversefinalroute=[]
    #q,r=crime(crimepath)
    flink=(crimelink(q,r, networkpath="C:\padmanabh\smartgwalior\smartcity/tempData\ROADNETWORK\ROADNETWORK.shp"))
    reverseroadId=[flink]
    ...
    x = True
    while x:
        final_route, firstcord, s = roadlink(n, m, fcord[0], fcord[1], id, l, reverseroadId,froute)
        if  s=="NOT OK FINISH LIST":
            print (s)
            break
...
def shortroute(c,d,q,r,froute):
    pathdics={}

    forwardroute=shortest_path_forward(c,d,q,r,froute)
    print("THE FORWARD ROUTE IS AS FOLLOWS",forwardroute)
    reverseroute=shortest_path_reverse(q,r,c,d)
    print("THE REVERSE ROUTE IS AS FOLLOWS",reverseroute,froute)
