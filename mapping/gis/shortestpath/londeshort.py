...
def circularring(crimepath,c,d):
    q,r=crime(crimepath)
    pointA=float((c+q)/2)
    pointB = float((d + r) / 2)
    point = ogr.Geometry(ogr.wkbPoint)
    ...
    name=[]
    for feature in layer:
        dic[feature.GetField("name")]=feature
        name.append(feature.GetField("name"))
    for i in name:
        dic[i]



def londeshort(crimepath,c,d):
    pathdics={}
    ...
    forwardroute=shortest_path_forward(serid,crimepath,float(tup[0]),float(tup[1]))
    print("THE FORWARD ROUTE IS AS FOLLOWS",forwardroute)
    reverseroute=shortest_path_reverse(crimepath,float(tup[0]),float(tup[1]),serid)
    print("THE REVERSE ROUTE IS AS FOLLOWS",reverseroute)
    l=commonid(forwardroute, reverseroute)
    #cpath=circularring(crimepath,c,d)
