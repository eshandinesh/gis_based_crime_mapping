...
def roadlink(pfeat, fcord, q, r, id, l, roadId, lit,dynadead):
    kdic = {}
    start_end_cord = {}
    liladhar = []
    fid = {}
...
def shortest_path_forward(q, r, c, d, cimelink, plink, policenodefeat, policenodecord, cimenodecord,dynadead):
    cordlist = []
    finalroute = []
    roadId = []
    ...
    while x:
        final_route, firstcord, s = roadlink(policenodefeat, policenodecord, float(cimenodecord[0]),
                                             float(cimenodecord[1]), cimelink, l, roadId, lit,dynadead)
...
def shortest_path_reverse(q, r, c, d, cimelink, plink, policenodefeat, policenodecord, cimenodecord,dynadead):
    cordlist = []
    finalroute = []
    revroadId = []
    l, lit = deadendremoval(q, r, c, d, cimelink, plink, cimenodecord, policenodecord)
    ...
    while x:
        final_route, firstcord, s = roadlink(policenodefeat, policenodecord, float(cimenodecord[0]),
                                             float(cimenodecord[1]), cimelink, l, revroadId, lit,dynadead)
        if s == "NOT OK FINISH LIST":
            # print(s)
            break
        policenodecord, policenodefeat = firstcord_1(firstcord[0], firstcord[1])
...
def bansaktahai(police, crime, plink, flink,a,dynadead):
    nextgen = {}
    jahis={}
    kash=[]
    crimeLink = None
...
def banakerahege(c,d,q,r,path,name):
    ...
    ford = [plink, flink]
    forwardroute, reverseroute=bansaktahai(police, crime, plink, flink,g,dynadead)
    l = commonid(forwardroute, reverseroute)
    if len(l) == 0:
    ...
    if len(l) > 0:
        ...
        return froute
pcr=[]
#banakerahege(78.19701642914038,26.21416090091164,78.19787352714707,26.21095042120316, "C:/Users\Hp\Desktop\DATA", "SHORTROUTE")
