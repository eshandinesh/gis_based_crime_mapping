...
def crime():
    datasource = open_File(path="C:\Users\Hp\Desktop\pol\CRIMEDATA\crime5.shp")
    numLayers = num_of_layers_in_file(datasource)
    layer = datasource.GetLayerByIndex(0)
    numFeatures = num_of_features_in_layer(datasource, numLayers)
    for featureIndex in range(numFeatures):
...
def nearest_policia(path):
    d={}
    patht={}
    l=[]
    crimedatasource = open_File("C:\Users\Hp\Desktop\pol\CRIMEDATA\crime5.shp")
...
def shortest_path():
    ...
    c,d = nearest_policia(path)
    #c,d=police()
    q,r = crime()
    id=crimelink(crimepath="C:\Users\Hp\Desktop\pol\CRIMEDATA\crime5.shp",networkpath="C:\Users\Hp\Desktop\pol\DATA\ROADCITYCENTER.shp")
